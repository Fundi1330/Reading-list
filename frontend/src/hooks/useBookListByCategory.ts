...
import { FinishedContext, InProcessContext, PlansContext, type FinishedContextType, type InProgerssContextType, type PlansContextType } from "../context/BooksContext";

export const useBookListByCategory = () => {
    const { plans, setPlans } = useContext<PlansContextType>(PlansContext);
    const { inProcess, setInProcess } = useContext<InProgerssContextType>(InProcessContext);
    const { finished, setFinished } = useContext<FinishedContextType>(FinishedContext);
